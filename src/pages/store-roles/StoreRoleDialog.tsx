...
import { storeRoleResolver, type StoreRoleDto, type StoreRole } from "@/schemas/storeRole.schema";
import { useStoreContext } from "@/stores/storeContext.store";

interface StoreRoleDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  selectedRole: StoreRole | null;
}

export function StoreRoleDialog({ open, onOpenChange, selectedRole }: StoreRoleDialogProps) {
  ...
  const { data: modulesData } = useQuery({
    ...
    queryFn: () =>
      import("@/services/store.service").then((m) => m.storeService.getModules(selectedStoreId!)),
  ...
  const { mutate: saveRole, isPending: isSaving } = useMutation({
    mutationFn: (data: StoreRoleDto) =>
      selectedRole
        ? storeRoleService.update(selectedStoreId!, selectedRole.id, data)
        : storeRoleService.create(selectedStoreId!, data),
    onSuccess: (res) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[640px] max-h-[80vh] h-[80vh] flex flex-col p-0 overflow-hidden text-left">
        <form onSubmit={handleSubmit((data: StoreRoleDto) => saveRole(data))} className="flex flex-col flex-1 min-h-0 overflow-hidden">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {modules.map((moduleItem) => {
                        ...
                        const isModuleAllSelected = apiCodes.every((code) => selectedPermissions.includes(code));
                        const isModuleSomeSelected = apiCodes.some((code) => selectedPermissions.includes(code)) && !isModuleAllSelected;
                            ...
                            <div className="flex items-center justify-between border-b pb-2">
                              ...
                              <div className="flex items-center space-x-2">
                                <Checkbox
                                  ...
                                  onCheckedChange={() => handleToggleModule(apiCodes, isModuleAllSelected)}
                                  className={isModuleSomeSelected ? "data-[state=unchecked]:bg-primary/20" : ""}
                                />
                                <label
